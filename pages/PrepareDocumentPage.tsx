            ...
            <div className="bg-surface rounded-3xl shadow-xl w-full max-w-2xl" onClick={e => e.stopPropagation()}>
                <div className="p-6 border-b border-outlineVariant">
                    <div className="flex justify-between items-center">
                        ...
                        <button onClick={onClose} className="p-1 rounded-full hover:bg-surfaceVariant" aria-label="Fermer"><X size={20} /></button>
                    </div>
                </div>
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6 max-h-[60vh] overflow-y-auto">
                    ...
                    <div className="space-y-4">
                        <div>
                            <h3 className="text-sm font-semibold text-onSurfaceVariant mb-3">DESTINATAIRES ({selectedCount}/{recipients.length} sélectionné{selectedCount > 1 ? 's' : ''})</h3>
                            <ul className="space-y-2">
                                {recipients.map(r => (
                                    <li key={r.id} className="flex items-start gap-3 p-3 rounded-lg border border-outlineVariant hover:bg-surfaceVariant/50 transition-colors">
                                        <input
                                            type="checkbox"
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-sm text-onSurfaceVariant truncate">{r.email}</p>
                                        </div>
                                    </li>
                                ))}
                            </ul>
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            <label htmlFor="creatorEmail" className="text-sm font-semibold text-onSurfaceVariant block mb-2">
...
const FieldPropertiesPanel: React.FC<{
    ...
    const colors = ['#3B82F6', '#10B981', '#F59E0B', '#EF4444', '#8B5CF6', '#71717A'];
    ...
    const currentColor = field.color || (recipient ? recipientColors[recipient.id % recipientColors.length] : '#71717A');

    return (
        <div>
            <div className="flex items-center gap-2 mb-4">
                <button onClick={onBack} className="p-2 rounded-full hover:bg-surfaceVariant"><ArrowLeft size={20}/></button>
                <h3 className="font-bold text-lg text-onSurface">Propriétés du champ</h3>
            </div>
            
            <div className="space-y-4 p-2">
                <div>
                    <label className="text-sm font-medium text-onSurfaceVariant">Type</label>
                    <p className="font-semibold text-onSurface">{field.type}</p>
                </div>
                <div>
                    <label className="text-sm font-medium text-onSurfaceVariant">Destinataire</label>
                    <p className="font-semibold text-onSurface">{recipient?.name || 'Non assigné'}</p>
                </div>

                <div className="grid grid-cols-2 gap-2">
                     <div>
                        <label htmlFor="width" className="text-sm font-medium text-onSurfaceVariant">Largeur (px)</label>
                        <input id="width" name="width" type="number" value={Math.round(field.width)} onChange={handleSizeChange} className="w-full text-sm p-2 bg-surfaceVariant/60 border border-outlineVariant rounded-lg mt-1 focus:ring-1 focus:ring-primary focus:border-primary focus:bg-surface" />
                     </div>
                     <div>
                        <label htmlFor="height" className="text-sm font-medium text-onSurfaceVariant">Hauteur (px)</label>
                        <input id="height" name="height" type="number" value={Math.round(field.height)} onChange={handleSizeChange} className="w-full text-sm p-2 bg-surfaceVariant/60 border border-outlineVariant rounded-lg mt-1 focus:ring-1 focus:ring-primary focus:border-primary focus:bg-surface" />
                     </div>
                </div>

                <div>
                    <label className="text-sm font-medium text-onSurfaceVariant mb-2 block">Couleur</label>
                    <div className="flex flex-wrap items-center gap-2">
                        {colors.map(c => (
                            <button key={c} aria-label={`Couleur ${c}`} onClick={() => handleColorChange(c)} className={`w-8 h-8 rounded-full transition-transform transform hover:scale-110 ${currentColor === c ? 'ring-2 ring-primary ring-offset-2 ring-offset-surface' : ''}`} style={{ backgroundColor: c }} />
                        ))}
                        <button onClick={() => onUpdate({ ...field, color: undefined })} className="text-xs text-onSurfaceVariant hover:underline ml-auto">Par défaut</button>
                    </div>
                </div>
            </div>
        </div>
...
const PrepareDocumentPage: React.FC = () => {
    ...
    const [editingField, setEditingField] = useState<{ index: number; action: 'move' | 'resize-br' } | null>(null);
    const [initialDragPosition, setInitialDragPosition] = useState<{ mouseX: number; mouseY: number; fieldX: number; fieldY: number; fieldW: number; fieldH: number } | null>(null);
    ...
    const resetState = () => {
        setFile(null); setFileBase64(''); setPdf(null); setPageDimensions([]);
        setTotalPages(0); setRecipients([]); setFields([]);
        setActiveRecipientId(null); setSelectedFieldType(null);
    }
    ...
    const loadPdfFile = async (selectedFile: File) => {
        ...
        if (isWordFile(selectedFile)) {
            ...
            try {
                ...
                addToast('Échec de la conversion. Veuillez exporter votre document en PDF depuis Word.', 'error');
                return;
            }
        }
        ...
        if (fileToProcess && fileToProcess.type === 'application/pdf') {
            ...
            reader.onload = async (event) => {
                ...
                try {
                    ...
                    addToast('Erreur lors du chargement du PDF. Le fichier est peut-être corrompu.', 'error');
                    console.error(error);
                    resetState();
                } finally {
            ...
            addToast('Veuillez sélectionner un fichier PDF ou Word (.doc, .docx).', 'info');
        }
    };
    ...
    const removeRecipient = (id: number) => {
        if (selectedFieldIndex !== null && fields[selectedFieldIndex]?.tempRecipientId === id) {
            setSelectedFieldIndex(null);
        }
        setRecipients(recipients.filter(r => r.id !== id));
        setFields(fields.filter(f => f.tempRecipientId !== id));
        if (activeRecipientId === id) {
            setActiveRecipientId(recipients.length > 1 ? recipients.find(r => r.id !== id)!.id : null);
        }
    };
    ...
    const handlePageClick = (e: React.MouseEvent<HTMLDivElement>, pageNum: number) => {
        ...
        const newField: TempField = {
            type: selectedFieldType, page: pageNum, x, y,
            width: selectedFieldType === FieldType.CHECKBOX ? 80 : (selectedFieldType === FieldType.DATE ? 86 : 110),
            height: selectedFieldType === FieldType.CHECKBOX ? 50 : (selectedFieldType === FieldType.DATE ? 50 : 50),
    ...
    // --- Field Drag & Resize ---
    const handleFieldMouseDown = (e: React.MouseEvent, index: number, action: 'move' | 'resize-br') => {
        e.stopPropagation();
        e.preventDefault();
    ...
    const handleMouseMove = (e: MouseEvent) => {
        ...
        setFields(currentFields => currentFields.map((field, index) => {
            if (index === editingField.index) {
                if (editingField.action === 'move') {
                    return { ...field, x: initialDragPosition.fieldX + dx, y: initialDragPosition.fieldY + dy };
    ...
    // --- Submission ---
    const sendEmailNotification = async (recipient: Recipient, token: string, documentName: string, subject: string, message: string) => {
    ...
    const handleConfirmSend = async (selectedRecipientIds: number[]) => {
        ...
        try {
            ...
            const selectedFields = fields.filter(f => selectedRecipientIds.includes(f.tempRecipientId));
            ...
            const emailPromises = envelope.recipients.map(recipient => {
                ...
                return tokenInfo ? sendEmailNotification(recipient, tokenInfo.token, envelope.document.name, emailSubject, emailMessage) : Promise.resolve({ success: false });
            });

            const emailResults = await Promise.all(emailPromises);
            const failedEmails = emailResults.map((r, i) => !r.success ? envelope.recipients[i].name : null).filter(Boolean);
            
            if (failedEmails.length > 0) {
                addToast(`Document envoyé, mais l'e-mail n'a pas pu être envoyé à : ${failedEmails.join(', ')}.`, 'error');
            } else {
                addToast(`Document et ${selectedRecipients.length} e-mail${selectedRecipients.length > 1 ? 's' : ''} envoyé${selectedRecipients.length > 1 ? 's' : ''} avec succès !`, 'success');
            }
            navigate('/dashboard');
        } catch (error) {
    ...
    const handleSend = () => {
        ...
        for (const recipient of recipients) {
            ...
            if (!recipient.email.trim()) {
                addToast(`Veuillez renseigner l'e-mail pour ${recipient.name}.`, 'error');
                return;
            }
            // Basic email format validation
            const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
            if (!emailRegex.test(recipient.email)) {
                addToast(`L'adresse e-mail "${recipient.email}" pour ${recipient.name} semble invalide.`, 'error');
                return;
            }
        }
        ...
        setEmailSubject(`Demande de signature : ${file.name}`);
    ...
    const handleGoToPage = (e: React.FormEvent) => {
        ...
        if (pdf && pageNum >= 1 && pageNum <= pdf.numPages) {
            ...
            addToast(`Veuillez entrer un numéro de page valide (1-${pdf?.numPages}).`, 'error');
        }
    };

    const PdfPageRenderer = useCallback(({ pageNum, currentZoom }: { pageNum: number, currentZoom: number }) => {
    ...
    const renderField = (field: TempField, index: number) => {
        ...
        const color = field.color || (recipient ? recipientColors[recipient.id % recipientColors.length] : '#71717A');
        ...
        return (
            <div 
                key={index} 
                style={baseStyle} 
                className={`rounded-md group p-1 flex flex-col justify-center items-center text-xs transition-shadow ${isSelected ? 'ring-2 ring-offset-2 ring-offset-white ring-primary shadow-lg' : ''}`}
            ...
            >
                ...
                <Icon style={{ color: color, width: `${14*zoomLevel}px`, height: `${14*zoomLevel}px`}} />
                <span style={{ color: color, fontSize: `${10*zoomLevel}px` }} className="font-bold truncate mt-0.5">{field.type}</span>
                <span style={{ color: color, fontSize: `${9*zoomLevel}px` }} className="font-medium truncate opacity-80">{recipient?.name || 'Non assigné'}</span>
                 <div
        ...
        return (
            <div className="container mx-auto max-w-3xl text-center">
                 <div className="bg-surface p-8 rounded-3xl shadow-sm border border-outlineVariant/30">
                    ...
                    <h1 className="text-3xl font-bold text-onSurface mt-4">Préparez votre document</h1>
                    <p className="mt-2 text-md text-onSurfaceVariant max-w-md mx-auto">
                        Téléversez un document PDF pour commencer à ajouter des destinataires et des champs de signature.
                    </p>
                    <div className="mt-8">
                            ...
                            <div className="relative border-2 border-dashed border-outlineVariant rounded-2xl p-12 hover:bg-surfaceVariant/50 transition-colors">
                                {isProcessing ? (
                                    <div className="flex flex-col items-center">
                                        ...
                                        <p className="mt-4 font-semibold text-onSurface">Traitement du PDF...</p>
                                    </div>
                                ) : (
                                    <div className="flex flex-col items-center">
                                        <UploadCloud className="h-10 w-10 text-onSurfaceVariant" />
                                        <span className="mt-4 font-semibold text-primary">Cliquez pour téléverser</span>
                                        <p className="text-sm text-onSurfaceVariant mt-1">ou glissez-déposez un fichier PDF ou Word ici</p>
                                    </div>
                                )}
                            </div>
                            <input id="file-upload" name="file-upload" type="file" className="sr-only" onChange={handleFileChange} accept=".pdf,.doc,.docx,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" disabled={isProcessing} />
                        </label>
                    </div>
                </div>
            </div>
                                ...
                                <div className="mt-6 border-t border-outlineVariant pt-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        {fieldTypeButtons.map(({ type, icon: Icon }) => (
                                            <button key={type} onClick={() => handleFieldTypeSelect(type)}
